...
def analytics_data_prep(query_return_df):
...
def analytics_data_query(area, unit, variable):
    db_conn = sqlite3.connect(db_name)
    selection = {'area': area, 'unit': unit, 'variable': variable, 'report_type': 'analytics'}
...
def analytics_focal_area_data_query(area, geoid, unit, variable):
    db_conn = sqlite3.connect(db_name)
    selection = {'area': area, 'geoid': geoid, 'unit': unit, 'variable': variable, 'report_type': 'analytics'}
...
def analytics_simple_lineplot_figure(area, geoid, unit, variable):
    # Get Statewide data.
    state_data = analytics_data_query(area, unit, variable)
    # Munge statewide data.
    state_result = analytics_data_prep(state_data)

    # Get selected geography data.
    geography_data = analytics_focal_area_data_query(area, geoid, unit, variable)
